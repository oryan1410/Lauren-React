...
export default function CigarCard(props) {
    ...
    return (
        <div className={`cardTwo ${props.cigar.inStock === false && 'outOfStock'}`}>
            ...
            <div className={`card2Content`}>
                <div className='card2-titleDiv'>
                    <span className='card2-title'>{props.title.length > 18 ? props.title.substring(0, 15) + '...' : props.title}</span><span className='card2-title hebTitle'>{props.cigar.Name_Heb.length > 20 ? props.cigar.Name_Heb.substring(0, 15) + '...' : props.cigar.Name_Heb}</span>
                </div>
            ...
            <div className={`card2-imageDiv ${isImageClicked ? 'slide' : ''}`} onClick={handleImageClick}>
                <div>
                    ...
                    <img src={props.cigar.ImageUrl} alt={`${props.title} bottle image`} className="card2image" />
                </div>
            </div>
        </div>
    );
