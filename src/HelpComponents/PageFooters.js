...
const {language} = useUserContext();
  ...
  return (
    ...
    // </div>
    //         </div>
    // <div className='pagefooter footer'>
    //   <div className='footerTop'>
    ...
    //       <Link className='footerLink' to='cigars'>{language==='en'?'Cigars':'סיגרים'}</Link>
    //       <Link className='footerLink' to='/events'>{language==='en'?'Events':'אירועים'}</Link>
    //         <Link className='footerLink' to='/about' >{language==='en'?'About Us':'אודות'}</Link>
    //         <Link className='footerLink' to='/contact' >{language==='en'?'Contact Us':'צור קשר'}</Link>
    //     <Link className='footerLink' to='/accessibility' >{language==='en'?'Accessibility Declaration':'הצהרת נגישות'}</Link>
    //         </div>
    //       <div className='footerUpperLeft'>
    //       <Link className='footerLink' to='/home'>{language==='en'?'Home':'בית'}</Link>
    //         <Link className='footerLink' to='/wines'>{language==='en'?'Wines':'יינות'}</Link>
    //         <Link className='footerLink' to='/dishes'>{language==='en'?'Menu':'תפריט'}</Link>
    //         <Link className='footerLink' to='/alcohol'>{language==='en'?'Alcohol':'אלכוהול'}</Link>
    //         <Link className='footerLink' to='/cocktails'>{language==='en'?'Cocktails':'קוקטיילים'}</Link>
    //       </div>
    //       </div>
    //   </div>
    // </div>
    <div style={{width:'100%', marginBottom:'3.55rem', overflow:'hidden'}}>
        <div className='pageFotterUpper'>
          <div className='footerUpperRight'>
            <Link className='footerLink' to='/about' >{language==='en'?'About Us':'אודות'}</Link>
            <Link className='footerLink' to='/contact' >{language==='en'?'Contact Us':'צור קשר'}</Link>
        <Link className='footerLink' to='/accessibility' >{language==='en'?'Accessibility Declaration':'הצהרת נגישות'}</Link>
            </div>
            <div className='footerUpperCenter'>
            <Link className='footerLink' to='/cocktails'>{language==='en'?'Cocktails':'קוקטיילים'}</Link>
            <Link className='footerLink' to='cigars'>{language==='en'?'Cigars':'סיגרים'}</Link>
            <Link className='footerLink' to='/facorites'>{language==='en'?'Favorites':'מועדפים'}</Link>
            <Link className='footerLink' to='/events'>{language==='en'?'Events':'אירועים'}</Link>
            </div>
          <div className='footerUpperLeft'>
          <Link className='footerLink' to='/home'>{language==='en'?'Home':'בית'}</Link>
